...
func uploadArtifact(ctx context.Context, httpClient *HttpClient, taskID string, localFilePath string) error {
	...
	// 调用 PostMultipart
	resp, err := httpClient.PostMultipart(uploadPath, textFields, fileFields)
	if err != nil {
...
func ExecuteAndStreamLogs(ctx context.Context, client pb.LogStreamServiceClient, cmdStr string, httpClient *HttpClient) error {
	...
	err = uploadArtifact(ctx, httpClient, ctx.Value("taskID").(string), fmt.Sprintf("backend/build/%s/linux-%s.tar.zst", ctx.Value("taskCommit").(string), ctx.Value("taskCommit").(string)))
	if err != nil {
